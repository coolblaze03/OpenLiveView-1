 ...
 */
package org.codechimp.openliveview.service;
...
import org.codechimp.openliveview.Preferences;
import org.codechimp.openliveview.MainActivity;
import org.codechimp.openliveview.R;
import org.codechimp.openliveview.data.LiveViewDbConstants;
import org.codechimp.openliveview.data.Prefs;
import org.codechimp.openliveview.messages.DecodeException;
...
import org.codechimp.openliveview.messages.calls.CapsRequest;
import org.codechimp.openliveview.messages.calls.DeviceStatusAck;
import org.codechimp.openliveview.messages.calls.DisplayBitmap;
import org.codechimp.openliveview.messages.calls.DisplayPanel;
import org.codechimp.openliveview.messages.calls.GetAlertResponse;
import org.codechimp.openliveview.messages.calls.GetTimeResponse;
import org.codechimp.openliveview.messages.calls.MenuItem;
import org.codechimp.openliveview.messages.calls.MessageAck;
import org.codechimp.openliveview.messages.calls.NavigationResponse;
import org.codechimp.openliveview.messages.calls.SetLed;
import org.codechimp.openliveview.messages.calls.SetMenuSettings;
import org.codechimp.openliveview.messages.calls.SetMenuSize;
import org.codechimp.openliveview.messages.calls.SetScreenMode;
import org.codechimp.openliveview.messages.calls.SetVibrate;
import org.codechimp.openliveview.messages.events.CapsResponse;
import org.codechimp.openliveview.messages.events.DeviceStatus;
import org.codechimp.openliveview.messages.events.GetAlert;
import org.codechimp.openliveview.messages.events.Navigation;
...
public class LiveViewThread extends Thread {
    ...
    private String PLUGIN_COMMAND = "org.codechimp.openliveview.plugin.command";
    private String PLUGIN_EVENT = "org.codechimp.openliveview.plugin.event";
    ...
    public LiveViewThread(LiveViewService parentService) {
        ...
        Intent notificationIntent = new Intent(parentService,
                MainActivity.class);
        PendingIntent pi_content = PendingIntent.getActivity(parentService, 0,
        ...
        bci.putExtras(bcb);
        PendingIntent pi_findliveview = PendingIntent.getBroadcast(
                parentService, 0, bci, 0);

        // Pending intent 3: Open the settings
        Intent i_opensettings = new Intent();
        i_opensettings.setClass(parentService, Preferences.class);
        PendingIntent pi_opensettings = PendingIntent.getActivity(
                parentService, 0, i_opensettings, 0);
        ...
        // Bitmap icon =
        // BitmapFactory.decodeResource(parentService.getResources(),
        // R.drawable.openliveview_icon);
        notification = new Notification.Builder(
                parentService.getApplicationContext())
                ...
                .addAction(
                        ...
                        parentService
                                .getString(R.string.notification_findliveview),
                        pi_findliveview)
                .addAction(
                        R.drawable.ic_menu_manage,
                        parentService.getString(R.string.notification_settings),
                        pi_opensettings).setOngoing(true).setWhen(0)
                .setPriority(Notification.PRIORITY_LOW).build();

        btAdapter = BluetoothAdapter.getDefaultAdapter();

        lvImage = loadImageByteArray(parentService, "menu_blank.png");
        lvImage_announce_android = loadImageByteArray(parentService,
                "announce_android.png");
        lvImage_announce_mail = loadImageByteArray(parentService,
                "announce_mail.png");
        lvImage_announce_note = loadImageByteArray(parentService,
                "announce_note.png");
        lvImage_announce_sms = loadImageByteArray(parentService,
                "announce_sms.png");
        lvImage_menu_battery = loadImageByteArray(parentService,
                "menu_battery.png");
        ...
        lvImage_menu_location = loadImageByteArray(parentService,
                "menu_location.png");
        ...
        lvImage_menu_notebook = loadImageByteArray(parentService,
                "menu_notebook.png");
        lvImage_menu_notification = loadImageByteArray(parentService,
        ...
        lvImage_menu_warning = loadImageByteArray(parentService,
                "menu_warning.png");
        lvImage_music_pause = loadImageByteArray(parentService,
                "music_pause.png");
        ...
        /*
         *
		 * menu_button_debug_id = menu_button_count; menu_button_count += 1;
		 */
    ...
    /**
     * This function was added by Basty149 Return byte array for the supplied
     * image file
     *
     * @param parentService
     ...
     */
    private byte[] loadImageByteArray(LiveViewService parentService,
        ...
        try {
            ...
            String message = "Error reading icon " + imageFileName + " : "
                    + e.getMessage();
            Log.e(TAG, message);
    ...
    public void run() {
        ...
        try {
            ...
            sendEvent("connectionstatus".toString(), 1, 0,
                    "connected".toString(), null);

            device_status = 2; // Bugfix: after connecting device is in menu
            // state.
		 ...
		 * runMinute, runtime); Log.d(TAG, message);
		 */
    ...
    private byte[] getImageForNotification(int type) {
        ...
        return lvImage_announce_android;
    } // Would like to use current "announce_android" image as
    // "announce_generic" and a different image for android notifications...
    ...
    private void processEvent(LiveViewEvent event) throws IOException {
        ...
        // Log.d(TAG, "openliveview will now process an event with id: " +
        // event.getId());
        switch (event.getId()) {
            case MessageConstants.MSG_GETCAPS_RESP:
                ...
                if (menu_state == 0) {
                    if (prefs.getInitialMenuItemId() > menu_button_count)
                        prefs.setInitialMenuItemId(menu_button_count);
                    if (prefs.getInitialMenuItemId() < 0)
                        prefs.setInitialMenuItemId(0);
                    if (prefs.getMenuVibrationTime() < 0)
                        prefs.setMenuVibrationTime(0);
                    if (prefs.getMenuVibrationTime() > 255)
                        prefs.setMenuVibrationTime(255);
                    sendCall(new SetMenuSettings(prefs.getMenuVibrationTime(),
                            prefs.getInitialMenuItemId()));
                ...
                if (menu_state == 0) {
                    for (byte current_id = 0; current_id <= menu_button_count; current_id++) {
                        if (menu_button_notifications_id == current_id) {
                            sendCall(new MenuItem(
                                    current_id,
                                    true,
                                    new UShort(
                                            (short) (parentService
                                                    .getNotificationUnreadCount(LiveViewDbConstants.ALERT_ALL))),
                                    "All notifications", lvImage_menu_notification));
                        }
                        if (menu_button_media_next_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Next", lvImage_menu_right));
                        }
                        if (menu_button_media_play_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Play / Pause", lvImage_menu_music));
                        }
                        if (menu_button_media_previous_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Previous", lvImage_menu_left));
                        }
                        if (menu_button_findphone_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Find my phone",
                                    lvImage_menu_location));
                        }
                        if (menu_button_battery_status_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) (get_battery_status() * 100)),
                                    "Battery", lvImage_menu_battery));
                        }
                        if (menu_button_plugintest_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Demo", lvImage_menu_debug));
                        }
                        if (menu_button_mediamenu_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Media menu", lvImage_menu_media));
                        }
                        if (menu_button_debug_id == current_id) {
                            sendCall(new MenuItem(current_id, false, new UShort(
                                    (short) 0), "Bitmap navigation test",
                                    lvImage_menu_debug));
                        }
                        if (menu_button_android_notifications_id == current_id) {
                            sendCall(new MenuItem(
                                    ...
                                    new UShort(
                                            (short) parentService
                                                    .getNotificationUnreadCount(LiveViewDbConstants.ALERT_ANDROID)),
                        ...
                        if (menu_button_sms_id == current_id) {
                            sendCall(new MenuItem(
                                    ...
                                    new UShort(
                                            (short) parentService
                                                    .getNotificationUnreadCount(LiveViewDbConstants.ALERT_SMS)),
                        ...
                        if (menu_button_notes_id == current_id) {
                            sendCall(new MenuItem(
                                    ...
                                    new UShort(
                                            (short) parentService
                                                    .getNotificationUnreadCount(LiveViewDbConstants.ALERT_NOTE)),
            ...
            case MessageConstants.MSG_GETALERT:
                ...
                Log.d(TAG, "Alert triggered (" + alert.getMenuItemId() + ", "
                        + alert.getAlertAction() + ", " + alert.getMaxBodySize()
                        + ")");
                ...
                if (alert.getAlertAction() == MessageConstants.ALERTACTION_NEXT) {
                    alertId += 1;
                    if (alertId > (byte) parentService
                            .getNotificationTotalCount(type) - 1) {
                        alertId = 0;
                    }
                }
                if (alert.getAlertAction() == MessageConstants.ALERTACTION_PREV) {
                    alertId -= 1;
                    if (alertId < 0) {
                        alertId = (byte) (parentService
                                .getNotificationTotalCount(type) - 1);
                    }
                }
                Log.d(TAG, "Notifications alert (ID: " + alertId + ") Time:"
                        + parentService.getNotificationTime(alertId, type));
                if (parentService.getNotificationTotalCount(type) > 0) {
                    String notificationTimeString = sdf.get().format(
                            new Date(parentService.getNotificationTime(alertId,
                                    type)));
                    sendCall(new GetAlertResponse(
                            (byte) parentService.getNotificationTotalCount(type),
                            (byte) parentService.getNotificationUnreadCount(type),
                            alertId, notificationTimeString,
                            parentService.getNotificationTitle(alertId, type),
                            parentService.getNotificationContent(alertId, type),
                            getImageForNotification(type)));

                    // if
                    // (parentService.getNotificationType(alertId)==LiveViewDbConstants.ALERT_SMS)
                } else {
                    sendCall(new GetAlertResponse(0, 0, alertId, "",
                            "No notifications", "", lvImage_announce_android));
                }
                break;
            case MessageConstants.MSG_NAVIGATION:
			/*
			 * Some random lines of code that I want to remember for later use:
			 * sendCall(new SetScreenMode((byte)
			 * MessageConstants.BRIGHTNESS_DIM)); sendCall(new
			 * SetLed(Color.GREEN,1,500)); sendCall(new
			 * NavigationResponse(MessageConstants.RESULT_CANCEL));
			 */
                Navigation nav = (Navigation) event;
                Log.d(TAG,
                        "Received navigation packet (isInAlert: " + nav.isInAlert()
                                + " and getNavType: " + nav.getNavType() + ").");
                if (nav.isInAlert()) {
                    Log.d(TAG,
                            "User pressed button in alert. Wiping all the notifications from the liveview...");
                    try {
                        ...
                        if (removeNotifications) {
                            ...
                            String notificationContentFilter = pref
                                    .getNotificationFilter();
                            String currentNotificationText = parentService
                                    .getNotificationContent(alertId,
                                            LiveViewDbConstants.ALERT_ALL);
                            pref.setNotificationFilter(notificationContentFilter
                                    + " " + currentNotificationText);
                        }
                    } catch (Exception e) {
                        String message = "Error while deleting all notifications from the database: "
                                + e.getMessage();
                        Log.e(TAG, message);
                    ...
                    switch (menu_state) {
                            ...
                            switch (nav.getNavType()) {
                                    ...
                                    switch (nav.getNavAction()) {
                                        case MessageConstants.NAVACTION_LONGPRESS:
                                            ...
                                            if (enable_media_menu) {
                                                sendCall(new NavigationResponse(
                                        ...
                                        default:
                                            Log.d(TAG,
                                                    "Navigation error: unknown action with select button!");
                                            sendCall(new NavigationResponse(
                                ...
                                case MessageConstants.NAVTYPE_MENUSELECT:
                                    ...
                                    if (nav.getMenuItemId() == menu_button_findphone_id) {
                                        Log.d(TAG,
                                                "Find my phone: generating noise and vibration...");
                                        Vibrator v = (Vibrator) parentService
                                        ...
                                        r.play();
                                        Log.d(TAG,
                                                "Find my phone: done, returning to menu.");
                                        sendCall(new NavigationResponse(
                                                MessageConstants.RESULT_CANCEL));
                                ...
                                case MessageConstants.NAVTYPE_DOWN:
                                    ...
                                    if (prefs.getMenuUpDownAction() == 1)
                                        action_volume_down();
                                    if (prefs.getMenuUpDownAction() == 2)
                                ...
                                case MessageConstants.NAVTYPE_UP:
                                    ...
                                    break;
                                default:
                                    String message = "Navigation error: unknown button ("
                                            + nav.getNavAction() + ")!";
                                    Log.e(TAG, message);
                            ...
                            switch (nav.getNavType()) {
                                case MessageConstants.NAVTYPE_SELECT:
                                    switch (nav.getNavAction()) {
                                        case MessageConstants.NAVACTION_LONGPRESS:
                                            Log.d(TAG,
                                                    "Long press on select key while in media menu.");
                                            menu_state = 0;
                                            sendCall(new NavigationResponse(
                                                    MessageConstants.RESULT_CANCEL));
                                            sendCall(new SetMenuSize(menu_button_count));
                                        ...
                                        case MessageConstants.NAVACTION_PRESS:
                                            ...
                                            sendCall(new NavigationResponse(
                                                    MessageConstants.RESULT_OK));
                                            Log.d(TAG,
                                                    "Navigation: Play / pause from media menu.");
                                            break;
                                        default:
                                            String message = "Navigation error: unknown action with select button while in media menu ("
                                                    + nav.getNavAction() + ")!";
                                            Log.e(TAG, message);
                                ...
                                case MessageConstants.NAVTYPE_LEFT:
                                    ...
                                    draw_media_menu();
                                    Log.d(TAG,
                                            "Navigation: Previous track from media menu.");
                                    break;
                        ...
                        case 2:
                            Log.e(TAG,
                                    "Menu state 2: Panel with information is visible. Returning to main menu.");
                            menu_state = 0;
                            sendCall(new NavigationResponse(
                                    MessageConstants.RESULT_CANCEL));
                            sendCall(new SetMenuSize(menu_button_count));
                        ...
                        case 3:
                            ...
                            switch (nav.getNavType()) {
                                ...
                                default:
                                    ...
                                    // sendCall(new
                                    // NavigationResponse(MessageConstants.RESULT_OK));
                                    break;
                            }
                            break;
                        case 4:
    ...
    public void draw_media_menu() throws IOException {
        ...
        AudioManager amgr = (AudioManager) parentService
                .getSystemService(Context.AUDIO_SERVICE);
        boolean playing = amgr.isMusicActive();
        if (playing) {
            Log.d(TAG, "A song is playing.");
            sendCall(new DisplayPanel(parentService.getMediaInfoTrack(),
                    "Artist: " + parentService.getMediaInfoArtist()
                            + " Album: " + parentService.getMediaInfoAlbum(),
                    lvImage_music_pause, false));
            // sendCall(new DisplayBitmap((byte) 34, (byte) 34,
            // menuImage_media_isplaying));
            // sendCall(new DisplayPanel("HEADER", "FOOTER",
            // menuImage_media_isplaying, false));
        } else {
            Log.d(TAG, "Nope, there is no music playing.");
            sendCall(new DisplayPanel("There is no music playing.", "",
                    lvImage_music_play, false));
            // sendCall(new DisplayBitmap((byte) 34, (byte) 34,
            // menuImage_media_isnotplaying));
        }
    }

    public void draw_battery_status() throws IOException {
        sendCall(new DisplayPanel("Battery status",
                (Math.round(get_battery_status() * 100)) + "%",
                lvImage_menu_battery, false));
    }

    public void pluginMenuAction(int menuId) throws IOException {
        sendCall(new NavigationResponse(MessageConstants.RESULT_OK));
        menu_state = 3;
        sendCall(new DisplayPanel("Press buttons!", "", lvImage_menu_music,
                false));
    ...
    public void showPanel(String top_string, String bottom_string,
                          boolean isAlert, boolean useImage, byte[] image) throws IOException {
        byte[] img;
        if (useImage) {
            img = image;
        } else {
    ...
    public void openMenuFromStandby() throws IOException {
        // sendCall(new DisplayPanel("Please wait...", "Loading...",
        // menuImage_notification, false));
        sendCall(new SetMenuSize(menu_button_count));
    }

    public void showIncomingCallScreen(int state, String topline,
                                       String bottomline) throws IOException {
        Prefs prefs = new Prefs(parentService);
        if (prefs.getEnableIncomingCallNotify()) {
            menu_state = 2;
            sendCall(new SetScreenMode((byte) MessageConstants.BRIGHTNESS_MAX));
            sendCall(new DisplayPanel(topline, bottomline, lvImage_menu_phone,
                    true));
        }
    ...
    public void action_volume_up() {
        AudioManager audioManager = (AudioManager) parentService
                .getSystemService(Context.AUDIO_SERVICE);
    ...
    public void stopLoop() {
        if (serverSocket != null) {
            try {
                ...
                String message = "Error while closing server socket: "
                        + e.getMessage();
                Log.e(TAG, message);
            }
        }
    ...
    public float get_battery_status() // This function was added by jkorp (user
    // on XDA)
    {
    ...
    public void emulate_media(int keycode) {
        ...
        buttonIntent.putExtra(Intent.EXTRA_KEY_EVENT, new KeyEvent(
    ...
    public void sendEvent(String event, int a, int b, String string1,
                          String string2) {
    ...
    public void showNewAlert(String line1, String line2, int icon_type,
                             byte[] img) {
        menu_state = 2;
    ...
    public void updateGuiAfterStandby() {
        try {
            ...
            Prefs prefs = new Prefs(parentService);
            Boolean update_unread_count_when_menu_opens = prefs
                    .getUpdateUnreadCountWhenMenuOpens();
            Boolean enable_notification_buzzer = prefs
                    .getenablenotificationbuzzer();

            if (update_unread_count_when_menu_opens
                    && (device_status == MessageConstants.DEVICESTATUS_ON)) {
                menu_state = 0; // Because that little piece of code commented
                // out below this does not work.
            ...
            if (enable_notification_buzzer
                    && (parentService
                    .getNotificationUnreadCount(LiveViewDbConstants.ALERT_ALL) > 0)
                    && (device_status == MessageConstants.DEVICESTATUS_ON)) {
                sendCall(new SetLed(Color.GREEN, 0, 1000));
                sendCall(new SetVibrate(0, 200));
            }
        } catch (Exception e) {
            String message = "Error while updating notifications: "
                    + e.getMessage();
            Log.e(TAG, message);
        }
    }
